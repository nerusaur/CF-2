...
Produces 4 figures saved to ../outputs/figures/:
    fig1_stratified_kfold_split.png   — fold structure (methodological transparency)
    fig2_gridsearch_heatmap.png       — CV mean & std across max_features x min_df
    fig3_accuracy_errorbar.png        — mean +/- std for all 16 configs
    fig4_best_config_folds.png        — per-fold accuracy of winning config

Research basis for each figure is cited inline.
...
# ── Style — scikit-learn / Rob Mulla clean defaults ──────────────────────
try:
...
# ── Text formula (must match preprocess.py) ──────────────────────────────
STOP_WORDS = {
...
le    = LabelEncoder()
y_enc = le.fit_transform(y_tr)   # 0=Educational, 1=Neutral, 2=Overstimulating


# =============================================================================
# FIGURE 1 — Stratified K-Fold Split Visualization
#
# Replicates the scikit-learn canonical split diagram (sklearn docs, plot_cv_indices).
# Shows that StratifiedKFold preserves class proportions in every fold — the
# methodological proof that results are not due to a lucky single split.
#
# Research basis:
#   Kohavi, R. (1995). A study of cross-validation and bootstrap for accuracy
#   estimation and model selection. IJCAI-95. (Stratified CV is the standard.)
# =============================================================================
...
for fold_i, (tr_idx, te_idx) in enumerate(skf_viz.split(X_tr, y_enc)):
    ...
    indices[te_idx]  = 1.0    # validation = warm
    indices[tr_idx]  = 0.0    # training   = cool
...
# Class distribution bar at bottom
ax1.scatter(range(len(y_enc)), [N_SPLITS + 0.5] * len(y_enc),
            c=y_enc, marker="|", lw=10, cmap=CLASS_CMAP,
            vmin=0, vmax=2, s=10)
...
# =============================================================================
# GRID SEARCH — runs all 16 max_features x min_df combinations
# =============================================================================
...
for i, mf in enumerate(max_features_opts):
    for j, md in enumerate(min_df_opts):
        ...
        mean_   = scores.mean()
        std_    = scores.std()
        rows.append((mf, md, mean_, std_))
        means[i, j] = mean_
        stds[i, j]  = std_

        if mean_ > best_mean:
            best_mean        = mean_
            best_cfg         = (mf, md, mean_, std_)
            best_fold_scores = scores.copy()

        if mf != prev_mf:
            print(f"  {'─'*60}")
            prev_mf = mf
        marker = "  <- BEST" if (mf, md) == (best_cfg[0], best_cfg[1]) else ""
        print(f"  {mf:>10}  {md:>7}  {mean_*100:>8.2f}%  +-{std_*100:.2f}%{marker}")
...
baseline = next((m*100 for mf, md, m, _ in rows if mf == 10000 and md == 2), None)
if baseline:
    print(f"\n  Baseline (10000, min_df=2) : {baseline:.2f}%")
    print(f"  Gain over baseline         : {best_cv*100 - baseline:+.2f}%")


# =============================================================================
# FIGURE 2 — Grid Search Heatmap (Mean + Std side by side)
#
# The canonical visualization for 2D hyperparameter search results.
# Dual heatmap — left shows accuracy, right shows variance — lets you
# see both dimensions of the accuracy-stability tradeoff at once.
#
# Research basis:
...
# Left panel: CV Mean
im1 = axes2[0].imshow(means * 100, cmap="YlGnBu",
                      vmin=np.min(means)*100 - 0.5,
                      vmax=np.max(means)*100 + 0.5,
                      aspect="auto")
for i in range(len(max_features_opts)):
    for j in range(len(min_df_opts)):
        star  = " *" if (max_features_opts[i]==best_mf and min_df_opts[j]==best_md) else ""
        color = "white" if means[i,j]*100 >= np.max(means)*100 - 0.8 else "black"
...
# Right panel: CV Std
im2 = axes2[1].imshow(stds * 100, cmap="OrRd",
                      vmin=0, vmax=np.max(stds)*100 + 0.3,
                      aspect="auto")
for i in range(len(max_features_opts)):
    for j in range(len(min_df_opts)):
        star  = " *" if (max_features_opts[i]==best_mf and min_df_opts[j]==best_md) else ""
        color = "white" if stds[i,j]*100 >= np.max(stds)*100 - 0.4 else "black"
...
# =============================================================================
# FIGURE 3 — Accuracy vs. Stability Error Bar Plot
#
# All 16 configs as bars with CV std error bars. The Step 4 baseline is drawn
# as a horizontal reference line. Shows which configs beat the baseline and
# whether the gain is stable or noisy.
#
# Research basis:
...
#   Error bars on CV scores are the standard reliability display.
# =============================================================================
print("Generating Figure 3: Accuracy vs stability error bar plot...")

fig3, ax3 = plt.subplots(figsize=(14, 6))

x_labels  = [f"mf={mf//1000}k / md={md}" for mf, md, _, _ in rows]
cv_means_ = [m * 100 for _, _, m, _ in rows]
cv_stds_  = [s * 100 for _, _, _, s in rows]
x_pos     = np.arange(len(rows))
md_colors = {1: "#2196F3", 2: "#4CAF50", 3: "#FF9800", 5: "#9C27B0"}
bar_cols  = [md_colors[md] for _, md, _, _ in rows]
...
# Highlight best
best_i = next(k for k, (mf, md, _, _) in enumerate(rows)
...
#
# Fold-by-fold breakdown of the winning config. The mean line and +-1 std
# shaded band show that no single fold is an outlier. This is the direct
# visual answer to "how do you know results are not due to a lucky split?"
#
# Research basis:
#   Bengio, Y. & Grandvalet, Y. (2004). No unbiased estimator of the
#   variance of k-fold cross-validation. JMLR, 5, 1089-1105.
#   Per-fold display is the standard way to show fold stability.
# =============================================================================
...
for v in fold_accs:
    if v == fold_accs.max():
        fold_colors.append(TEST_COLOR)
    elif v == fold_accs.min():
        fold_colors.append(TRAIN_COLOR)
    else:
        fold_colors.append("#5BA4CF")
...
ax4.legend(handles=color_patches, fontsize=9,
...
print(f"  fig3  Accuracy vs stability      -> error bar comparison vs baseline")
...
print(f"    CV Mean           = {best_cv*100:.2f}%  +-{best_std*100:.2f}%")
print(f"\n-> Open 5final_eval.py and set these three values at the top.")
